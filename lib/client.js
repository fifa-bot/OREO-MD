...
try {
    ...
    console.log("ðŸŒ Connected to the OREO  BOT")
} catch {
...
let cc = Config.sessionName.replace(/VAJIRA;;;/g, "").replace(/OREO;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
                        ...
                        if (citel.isGroup && !isAdmins && mongoschema == 'true') {
                            ...
                            array.map(async(bg) => {
                                ...
                                if (pattern.test(chab)) {
                                    if (!isBotAdmins) {
                                        let buttonMessage = {
                                            text: `*Â«Â«Â«Â«Link has been detected  Â»Â»Â»*
...
Don't tag him,he is busy now. But Don't worry I assure you,I'll inform him As soon as possibleðŸ˜.
 ...
 Void.ev.on('group-participants.update', async(anu) => {
                    ...
                    if (checkinfo) {
                        ...
                        if(/@pp/g.test(goodbye_messages)) {
                           let buttonMessage = {
                            ...
                            let buttonMessage = {
                                image: {url : ppUrl },
                                caption: `[ ð—£ð—¥ð—¢ð— ð—¢ð—§ð—˜ ð—›ð—”ð—¦ ð—•ð—˜ð—˜ð—¡  ð——ð—˜ð—§ð—˜ð—–ð—§ð—˜ð—— ]\n\nName : @${num.split("@")[0]}\nStatus : from Member â—? to Admin\nGroup : ${metadata.subject}`,
                                footer: `mac`,
                                mentions: [num],
                                headerType: 4,
                            }
                            ...
                            let buttonMessage = {
                                image: { url : ppUrl },
                                caption: `[ *HEY SADLY A* ð˜¿ð™€ð™ˆð™Šð™ð™€ ð™ƒð˜¼ð™Ž ð˜½ð™€ð™€ð™‰  ð˜¿ð™€ð™ð™€ð˜¾ð™ð™€ð˜¿ ]\n\nName : @${num.split("@")[0]}\nStatus :from Admin â—? to Member`,
                                footer: `mac`,
                                mentions: [num],
                                headerType: 4,
                            }
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === "connecting") {
                   console.log("ðŸ“¡ Connecting to WhatsApp... Please Wait.");
                }
                if (connection === 'open') {
                    console.log("âœ? Oreo Login Successful!");
                    console.log("â¬‡ï¸  Oreo Installing External Plugins...");
                    ...
                    console.log("ðŸ˜  Oreo External Plugins Installed!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
...
app.listen(port, () => console.log(`OREO MD Server listening on port http://localhost:${port}!`));
